...
module edb_la_top #(
    ...
    localparam _64BIT_PER_WORD = (CAPTURE_WIDTH - 1) / 64 + 1;
    // Remarks:
    ...
    always @(posedge bscan_TCK or posedge bscan_RESET) begin
        ...
        else if (cap_buf_read_done) begin
            la_run_trig      <= 1'b0;
            la_run_trig_imdt <= 1'b0;
            la_stop_trig     <= 1'b0;
        end 
    ...
    always @(posedge clk or posedge bscan_RESET) begin 
        if (bscan_RESET) begin
            la_resetn_p1   <= 1'b0;
            la_resetn      <= 1'b0;
        end 
    ...
    generate
        if (TRIGIN_EN) begin
            always @(posedge clk or negedge la_resetn) begin
                if (!la_resetn) begin
                    trig_in_ack <= 1'b0;
                end 
                else begin
    ...
    la_biu #(
        ...
        .cap_buf_read_done  ( cap_buf_read_done ),
        .trig_out           ( trig_out ),
...
module la_biu #(
    ...
    always @(posedge clk or negedge la_resetn) begin
        if (!la_resetn) begin
            ...
            cap_buf_read_done_p1 <= 1'b0;
            cap_buf_read_done_p2 <= 1'b0;
            cap_buf_read_done_p3 <= 1'b0;
        end
        else begin
            ...
            cap_buf_read_done_p1 <= cap_buf_read_done;
            cap_buf_read_done_p2 <= cap_buf_read_done_p1;
            cap_buf_read_done_p3 <= cap_buf_read_done_p2;
        end
    end

    assign trig_pos_reached = (pos_counter == la_trig_pos - 1);

    assign cap_buf_read_done_negedge = cap_buf_read_done_p3 && !cap_buf_read_done_p2;
    ...
    always @(*) begin
        case(curr_state)
            ...
            LA_FULL: begin
                if (cap_buf_read_done_negedge) begin
                    next_state = LA_IDLE;
                end else begin
                    next_state = LA_FULL;
                end
